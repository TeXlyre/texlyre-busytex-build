...
CFLAGS_TEXLIVE_wasm     = -I$(abspath build/wasm/texlive/libs/icu/include)   -I$(abspath source/fontconfig) -I$(abspath build/wasm/fontconfig) $(CFLAGS_OPT_wasm) -sERROR_ON_UNDEFINED_SYMBOLS=0 -Wno-error=unused-but-set-variable
CXXFLAGS_TEXLIVE_wasm   = $(CFLAGS_TEXLIVE_wasm)
CFLAGS_TEXLIVE_native   = -I$(abspath build/native/texlive/libs/icu/include) -I$(abspath source/fontconfig) -I$(abspath build/native/fontconfig) $(CFLAGS_OPT_native)
...
build/texlive-%.txt: build/texlive-%.profile source/texmfrepo.txt
	...
	echo '<?xml version="1.0"?><!DOCTYPE fontconfig SYSTEM "fonts.dtd"><fontconfig><dir prefix="relative">texmf-dist/fonts/opentype</dir><dir prefix="relative">texmf-dist/fonts/type1</dir></fontconfig>' > $(basename $@)/fonts.conf
...
build/collection-%.txt: build/collection-%.profile source/texmfrepo.txt
	...
	echo '<?xml version="1.0"?><!DOCTYPE fontconfig SYSTEM "fonts.dtd"><fontconfig><dir prefix="relative">texmf-dist/fonts/opentype</dir><dir prefix="relative">texmf-dist/fonts/type1</dir></fontconfig>' > build/collection-$*/fonts.conf
